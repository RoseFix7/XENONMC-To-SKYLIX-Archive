...
export default class InitHandle {
	...
	public constructor() {
		program.command("init [path]").action((initPath) => {
			...
			this.askAllInfo(async (projectAnswers) => {
				...
				const packageFile = await this.generatePackageJSON(projectAnswers);
	 ...
	 */
	private async generatePackageJSON(projectData: InitAnswers) {
		const projectPkg = {
		...
		if (projectData.typeScript && projectData.type != AppConfigType.node) {
			projectPkg.devDependencies["typescript"] = await this.getPackageVersions(
				"microsoft",
				"typescript"
			);
		}
		...
		if (projectData.type == AppConfigType.desktop) {
			...
			projectPkg.dependencies["@skylixgh/nitrojs-desktop-electron"] = pkg.version;
			projectPkg.dependencies["@skylixgh/nitrojs-desktop-uix"] = pkg.version;

			projectPkg.devDependencies["vite"] = await this.getPackageVersions("vitejs", "vite");
			projectPkg.dependencies["react"] = await this.getPackageVersions("facebook", "react");
			projectPkg.dependencies["react-dom"] = await this.getPackageVersions("facebook", "react");

			projectPkg.devDependencies["electron-builder"] = await this.getPackageVersions(
				"electron-userland",
				"electron-builder"
			);
	...
	/**
	 * Get all tags of a package from github
	 * @param org The org name
	 * @param repo The repo name
	 * @returns Promise containing latest package version
	 */
	public getPackageVersions(org: string, repo: string): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			https
				.request(
					...
					(response) => {
						response.on("data", (chunk) => (fullRes += chunk.toString()));

						response.on("end", () => {
							try {
								const tags = JSON.parse(fullRes);
								resolve(tags[0].name.substring(1));
							} catch (error) {
								reject(error);
							}
						});
